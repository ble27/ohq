 ...
 */
async function authorizeQueueManagerForTicket(ticketId, userId) {
    ...
    if (!allowed)
        throw new Error('You do not manage this queue');
    return queue;
}
const app = express();
...
io.on('connection', async (socket) => {
    ...
    const reportSocketError = (event, error) => {
    ...
    socket.on('watch-queue', async (queueId) => {
        try {
    ...
    socket.on('unwatch-queue', async (queueId) => {
        try {
    ...
    socket.on('refresh-queue', async (queueId) => {
        try {
            const tickets = await listActiveTickets(queueId);
            io.to(queueId).emit('queue-updated', tickets);
        }
    ...
    socket.on('start-helping', async (ticketId) => {
        try {
    ...
    socket.on('complete-ticket', async (ticketId) => {
        try {
            const queue = await authorizeQueueManagerForTicket(ticketId, userId);
