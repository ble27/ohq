...
const isUniqueConstraintViolation = (error) => error instanceof Prisma.PrismaClientKnownRequestError && error.code === 'P2002';
const ACTIVE_STATUSES = [SessionStatus.WAITING, SessionStatus.HELPING];
...
export const listActiveTickets = async (queueId) => {
...
export const joinQueue = async (queueId, studentId) => {
...
export const leaveQueue = async (queueId, studentId) => {
...
export const startHelping = async (ticketId) => {
    await closeExpiredQueues();
...
export const removeFromQueue = async (ticketId) => {
    const existing = await prisma.queueTicket.findUnique({ where: { id: ticketId } });
    if (!existing) {
        throw new Error('Ticket not found');
    }
