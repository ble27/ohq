...
import { CreateQueueValidationSchema, QueueOpenParamSchema, RoomLocationParamSchema, TimeValidationSchema, } from '../schemas/queue.schema.js';
...
router.get('/', async (req, res) => {
    try {
        ...
        const body = { queues, message: 'SUCCESS' };
...
router.get('/:id', async (req, res) => {
    try {
        ...
        const body = { queue, message: 'SUCCESS' };
...
router.post('/', requireRole(Role.TA, Role.PROFESSOR), async (req, res) => {
    try {
        ...
        const body = { queue: newQueue, message: 'SUCCESS' };
...
router.patch('/:queueId/status/:isQueueOpen', requireQueueOwnership('queueId'), async (req, res) => {
    try {
...
router.patch('/:queueId/location/:roomLocation', requireQueueOwnership('queueId'), async (req, res) => {
    try {
        const queueId = req.params.queueId;
        const roomLocation = RoomLocationParamSchema.parse(decodeURIComponent(req.params.roomLocation));
        const result = await prisma.queue.update({
            where: { id: queueId },
...
router.patch('/:id', requireQueueOwnership('id'), async (req, res) => {
    try {
...
router.patch(`/:queueId/time`, requireQueueOwnership('queueId'), async (req, res) => {
    try {
        const queueId = req.params.queueId;
...
router.delete('/:id', requireQueueOwnership('id'), async (req, res) => {
    try {
        // requireQueueOwnership already fetched and authorized the queue.
