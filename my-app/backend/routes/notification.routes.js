...
import { filterRecipientsByNotificationPreference, isNotificationEnabledForUser, } from '../services/notification.services.js';
...
router.post('/queues/:queueId/user/:recipientId/type/:type', async (req, res) => {
    try {
...
router.post('/queues/:queueId/type/close', requireQueueOwnership('queueId'), async (req, res) => {
    try {
        const queueId = req.params.queueId;
        ...
        const enabledRecipientIds = await filterRecipientsByNotificationPreference(recipientIds, type);
        ...
        const response = await prisma.$transaction(enabledRecipientIds.map((userId) => prisma.notification.create({
...
router.delete('/user/:userId', requireSelf('userId'), async (req, res) => {
    ...
    try {
        ...
        console.log(`Successfully cleared all notifications associated with user ${userId}`, JSON.stringify(body, null, 2));
        res.status(200).json(body);
    }
    catch (error) {
        if (error instanceof Error) {
