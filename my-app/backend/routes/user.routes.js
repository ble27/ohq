...
const router = Router();
// GET /api/users/:id
router.get('/:id', requireSelf('id'), async (req, res) => {
    try {
...
router.post('/', async (req, res) => {
    try {
        ...
        const body = { user: newUser, message: 'User created' };
...
router.patch('/:id/name', requireSelf('id'), async (req, res) => {
    try {
...
router.patch('/:id/notifications/type/:type', requireSelf('id'), async (req, res) => {
    try {
...
router.patch('/:id/notifications/sound', requireSelf('id'), async (req, res) => {
    try {
        const id = req.params.id;
        const { status } = NotificationAlertUpdateSchema.parse(req.body);
        const response = await prisma.user.update({
...
router.patch('/:id/defaultlocation', requireSelf('id'), async (req, res) => {
    try {
        const id = req.params.id;
...
// DELETE /api/users/:id
router.delete('/:id', requireSelf('id'), async (req, res) => {
    try {
        const userId = req.params.id;
        if (!userId) {
            res.status(400).json({ message: 'User ID is required' });
